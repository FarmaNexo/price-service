...
func NewSQSEventPublisher(
	...
	logger.Info("SQS EventPublisher inicializado",
		...
		zap.String("queue_url", sqsCfg.PriceEventsQueueURL),
		zap.String("endpoint", awsCfg.Endpoint),
	)

	return &SQSEventPublisher{
		sqsClient: sqsClient,
		queueURL:  sqsCfg.PriceEventsQueueURL,
		logger:    logger,
	}, nil
}

// Publish publica un evento de precios en SQS
func (p *SQSEventPublisher) Publish(ctx context.Context, event events.PriceEvent) error {
